...
from cykit.models import ConnectionOptions, DeviceInfo, Model, Transport
...
def test_parse_legacy_config_normalizes_mixed_case_bluetooth_key() -> None:
    connection, _, _ = parse_legacy_config("Bluetooth=aabbccdd+NoWeb")
    assert connection.device_key == "AABBCCDD"
    assert connection.transport == Transport.BLUETOOTH
...
def test_cli_rejects_invalid_model() -> None:
    exit_code = cli(["run", "127.0.0.1", "54123", "9", "--noweb"])
    assert exit_code == 2
...
def test_discover_text_output(
    ...
    assert "serial=None" in output
    assert "gatt_match" not in output
    assert "address=" not in output


def test_client_reuses_bluetooth_device_info_without_rediscovery(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    client = CyKitClient(
        4, connection=ConnectionOptions(transport=Transport.BLUETOOTH, device_key="AABBCCDD")
    )

    class FakeEEG:
        serial_number = "SERIAL-123456789"

    monkeypatch.setattr(client, "_eeg", FakeEEG(), raising=False)
    monkeypatch.setattr("cykit.client.eeg.BTLE_device_name", "Insight", raising=False)
    monkeypatch.setattr(
        "cykit.client.discover",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("discover should not be called")
        ),
    )

    client._resolved_device_info = client._build_resolved_device_info()
    info = client._resolve_device_info()

    assert info.transport == Transport.BLUETOOTH
    assert info.device_key == "AABBCCDD"
    assert info.name == "Insight"
