...
class socketIO:
    def __init__(self, port, uid, ioHandler):
        ...
        self.thread_2 = threading.Thread(name="ioThread", target=self.run, daemon=True)
    ...
    def Connect(self):
        ...
        if _info_is_true(self.io, "noweb"):
            time.sleep(0)
            return "noweb"
    ...
    def run(self):
        ...
        if _info_is_true(self.io, "noweb"):
            time.sleep(0.001)
            while self.socketThreadRunning:
                if not _info_is_true(self.io, "status"):
                    return
                time.sleep(0.001)
                continue
            return

        if _info_is_true(self.io, "openvibe"):
            time.sleep(0.001)
            self.io.onGeneric(0)
        ...
        while self.socketThreadRunning:
            ...
            if _info_is_true(self.io, "generic"):
                try:
                ...
                try:
                    ...
                    if ready[0]:
                        ...
                        resKey = base64.b64encode(
                            hashlib.new(
                                "sha1",
                                (secKey + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11").encode("utf-8"),
                            ).digest()
                        )
                        ...
                        response += f"""Sec-WebSocket-Accept: {resKey}\r\n\r\n"""
                        try:
                ...
                try:
                    ...
                    if ready[0]:
                        ...
                        if data_length <= 125:
                            ...
                            payloadLength = struct.unpack(
                                "H", bytes(self.con.recv(2).decode("latin-1"), "latin-1")
                            )[0]
                        elif data_length == 127:
                            payloadLength = struct.unpack(
                                "Q", bytes(self.con.recv(8).decode("latin-1"), "latin-1")
                            )[0]
    ...
    def onClose(self, location):
        ...
        if (
            ...
            and "closed" not in str(self.con)
            and not _info_is_true(self.io, "noweb")
        ):
            self.con.close()
    ...
    def packData(self, text):
        ...
        data = f"{self.uid}<split>{sign}<split>{text}"
        return data
    ...
    def sendData(self, text):
        ...
        if self.uid == 0:
            ...
            try:
                ...
                self.con.send(struct.pack(f"!{length}s", bytes(text, "utf-8")))
